...
public static class Plant {
   ...
   private static void GrassGen(int x, int y, int z){
      if (!CanPlant(x, y - 1, z)) return;
      if (Random.value < 0.1 && x > 0 && x < TotalLength() - 1 && z > 0 && z < TotalLength() - 1 && y < Tweaks.chunkHeight - 2){
   ...
   private static void FlowersGen(int x, int y, int z){
      if (!CanPlant(x, y - 1, z)) return;
      if (Random.value < 0.05 && x > 0 && x < TotalLength() - 1 && z > 0 && z < TotalLength() - 1 && y < Tweaks.chunkHeight - 2){
